...
public class CategoryEventAction extends AjaxBaseAction {
	...
	/**
	 * @param cateEvents the cateEvents to set
	 */
	...
	/**
	 * @param cateEvent the cateEvent to set
	 */
	...
	public String index(){
		...
		cateEvents = this.categoryEventService.getEntitiesByParamsAndPages(params, page.getStartData(), pageSize);
		return SUCCESS;
	}
	...
	public String save(){
		if (categoryEventService.isExistByName(cateEvent.getCgeName(), cid)) {
			...
			this.setAjaxJson(this.getText("npo.config.category.save.name.exist", args), "", "", "", "");
			return AJAXERROR;
		}
		if (cid == null) {
			if (this.categoryEventService.isExistById(cateEvent.getCgeId())) {
				String[] args = { cateEvent.getCgeId().toString() };
				this.setAjaxJson(this.getText("npo.config.category.save.id.exist", args), "", "", "", "");
				return AJAXERROR;
			}
