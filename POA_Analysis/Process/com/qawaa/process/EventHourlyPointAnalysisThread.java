...
import com.qawaa.control.EventDailyPointAnalysisControl;
import com.qawaa.control.EventHourlyPointAnalysisControl;
...
public class EventHourlyPointAnalysisThread extends Thread implements Runnable {
	private static Log LOG = LogFactory.getLog(EventHourlyPointAnalysisThread.class);
	...
	private EventHourlyPointAnalysisControl ehpaControl = EventHourlyPointAnalysisControl.getInstance();
		...
		while (flag) {
			try {
				...
				if (event == null) {
					...
					LOG.info("Analysis event[" + event_index + "] hourly finish, wait for next event...");
				}
	...
	private void analysis(){
		Date date;
		PointMultipleHourly hourlyPoint = ehpaControl.getLastPointMultipleHourlyByEvent(event.getEventId());
	...
	private Date save(Date date){
		PointMultipleDaily dailyPoint = edpaControl.getPointMultipleDailyByEventAndDate(event.getEventId(), DateUtil.getBeginTimeOfDay(date));
		if(dailyPoint == null)
			return null;
		...
		PointMultipleHourly hourlyPoint = ehpaControl.getPointMultipleHourlyByEventAndDate(event.getEventId(), DateUtil.getBeginTimeOfHour(date));
		if(hourlyPoint == null){
			hourlyPoint = new PointMultipleHourly();
			hourlyPoint.setMhpDate(new Timestamp(DateUtil.getBeginTimeOfHour(date).getTime()));
		...
		LOG.info("Analysis event["+ hourlyPoint.getMhpEventId() +"] hourly point["+ hourlyPoint.getMhpPoint() +"] at " + hourlyPoint.getMhpDate());
		return DateUtil.getBeginTimeOfHour(new Date(hourlyPoint.getMhpDate().getTime() + 65L*60L*1000L));
	}
